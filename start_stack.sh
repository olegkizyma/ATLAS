...
free_port_if_requested() {
    ...
    if [ "${FORCE_FREE_PORTS:-false}" = "true" ]; then
        ...
        if [ -x "scripts/kill_port.sh" ]; then
            ...
            # М'яке завершення, потім жорстке
            local pids
...
if lsof -ti:3000 > /dev/null 2>&1; then
    if [ "${FORCE_FREE_PORTS:-false}" = "true" ]; then
        ...
        # Спробуємо визначити, чи це вже запущений Goose
...
# 2.5 Локальний Fallback LLM (Port 3010) — Optional: запускаем только по флагу, 3010 считаем внешним провайдером
